...
public class CryptoTools {
    public static final double[] ENGLISH = {8.12, 1.49, 2.71, 4.32, 12.02, 2.3, 2.03, 5.92, 7.31, 0.1, 0.69, 3.98,
    ...
    /**
     * Clean away all the non-letters from an ANSI byte array and return a new byte
     * array with letters converted to upper-case.
     **/
    ...
    public static String bytesToBin(byte[] data) {
        ...
        for (byte b : data) {
            ...
            while (tmp.length() < BITS_PER_BYTE)
                tmp = "0" + tmp;
            result.append(tmp);
     ...
     **/
    public static double[] getRelativeFrequencies(byte[] ar) {
        int[] count = getFrequencies(ar);
